...
export class GaussianKnowledgeService extends Service {
  static serviceType = "gaussian-knowledge";
  capabilityDescription =
    "Automatically monitors example_logs directory, parses Gaussian files, and maintains a persistent knowledge graph for querying";
  ...
  static async start(runtime: IAgentRuntime): Promise<GaussianKnowledgeService> {
    const service = new GaussianKnowledgeService(runtime);
    await service.initialize(runtime);
    console.log("🧠 Gaussian Knowledge Service started - monitoring example_logs/");
    return service;
  }
  ...
  async initialize(runtime: IAgentRuntime): Promise<void> {
    ...
    try {
      ...
      await fs.mkdir(path.dirname(this.knowledgeGraphPath), {
        recursive: true,
      });
      ...
      console.log(`📊 Knowledge graph initialized: ${this.knowledgeGraphPath}`);
      console.log(`👀 Watching directory: ${this.watchedDirectory}`);
    } catch (error) {
      console.error(
        "❌ Failed to initialize Gaussian Knowledge Service:",
        error,
      );
    }
  }
  ...
  async stop(): Promise<void> {
    console.log("🧠 Gaussian Knowledge Service stopped");
  }
  ...
  private async loadOrCreateKnowledgeGraph(): Promise<void> {
    try {
      ...
      if (stats.isFile()) {
        ...
        console.log(`📊 Loaded knowledge graph (${content.length} bytes)`);
      }
    } catch (error) {
...
`;
    ...
    console.log("✅ Initial knowledge graph created");
  }
  ...
  private async scanExistingFiles(): Promise<void> {
    try {
      ...
      console.log(`🔍 Found ${gaussianFiles.length} Gaussian files to process`);

      for (const file of gaussianFiles) {
        await this.processGaussianFile(path.join(this.watchedDirectory, file));
      }

      console.log(`✅ Processed ${gaussianFiles.length} existing files`);
    } catch (error) {
      ...
      async (eventType, filename) => {
        ...
        if (eventType === "rename" || eventType === "change") {
          try {
            ...
            console.log(`🆕 New Gaussian file detected: ${filename}`);
            await this.processGaussianFile(filePath);
          } catch (error) {
            // File was deleted or is inaccessible
            console.log(`🗑️  File removed or inaccessible: ${filename}`);
          }
        }
      },
    );
  ...
  private async processGaussianFile(filePath: string): Promise<void> {
    try {
      ...
      console.log(
        `✅ Added ${tripleCount} triples from ${path.basename(filePath)}`,
      );
    } catch (error) {
  ...
  private async appendToKnowledgeGraph(
    rdfContent: string,
    sourcePath: string,
  ): Promise<void> {
    try {
      ...
      console.error("❌ Error appending to knowledge graph:", error);
    }
  }
  ...
  async queryKnowledgeGraph(query: string): Promise<any> {
    try {
      ...
      const stats = {
        ...
        molecules: (content.match(/ontocompchem:QuantumCalculation/g) || [])
          .length,
      ...
      const relevantLines = lines.filter(
        (line) =>
          ...
          (queryLower.includes("homo") &&
            line.includes("ontocompchem:hasHOMOLUMOGap")) ||
          (queryLower.includes("frequency") &&
            line.includes("ontocompchem:hasFrequency")) ||
    ...
    try {
      ...
      return {
        ...
        molecules: (content.match(/ontocompchem:QuantumCalculation/g) || [])
          .length,
        scfEnergies: (content.match(/ontocompchem:hasSCFEnergy/g) || []).length,
        homoLumoGaps: (content.match(/ontocompchem:hasHOMOLUMOGap/g) || [])
          .length,
        frequencies: (content.match(/ontocompchem:hasFrequency/g) || []).length,
        atoms: (content.match(/cheminf:hasAtom/g) || []).length,
