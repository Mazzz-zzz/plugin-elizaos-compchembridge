...
export class GaussianParserService extends Service {
  ...
  capabilityDescription =
    "Enables the agent to parse Gaussian 16 quantum chemistry logfiles into RDF knowledge graphs using standard ontologies";
  ...
  static async start(runtime: IAgentRuntime): Promise<GaussianParserService> {
    ...
    const pythonPath =
      runtime.getSetting("GAUSSIAN_PARSER_PYTHON_PATH") || "python";
    const maxFileSize =
      runtime.getSetting("GAUSSIAN_PARSER_MAX_FILE_SIZE") || "500";
  ...
  async validateGaussianFile(filePath: string): Promise<boolean> {
    try {
      ...
      return gaussianSignatures.some((signature) =>
